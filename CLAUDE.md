...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Continuar Track 5: AI 5 agentes (tool use / ReAct) → AI 6 evals → capstone "Nebula RAG" (el corpus de Nebula ya aparece en AI 4). Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
