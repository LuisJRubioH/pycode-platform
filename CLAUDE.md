...
**Contenido en números**: 53 lecciones (Track 0: 5 · Track 1: 10 · Track 2: 11 · Track 3: 11 · Track 4: 5 · Track 5: 6 · Track 6: 5) · 228 ejercicios (198 de código + 30 de Track 0, que no se ejecutan) (todos con hidden_tests; Track 1 reescrito entero: 60) · 100 puzzles ELO curados · 85 retos (25 problemas × 3 niveles, 5 de ellos de datos/ML, + 10 curados; todos con hidden_tests y solución de referencia) · 6 capstones · 3 datasets. Migraciones 0001-0016 (Tracks 3-6 y el proxy LLM **no** añaden migraciones; la 0015 es `coding_challenges.hidden_tests` y la 0016 los tipos de ejercicio de Track 0). 228 tests backend. Esquema de datos: `docs/DATABASE.md`. **Nota**: el contenido real vive en `lesson_seed.py`; `lesson_content.py` es código **muerto/duplicado** (no se importa) — no editarlo pensando que seedea.
...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Track 5 quedó cerrado con el capstone "Nebula RAG". **Track 6 cerrado** (MLOps 1-5 + capstone "pipeline de producción"). **Track 0 en piloto**: infra + lecciones 1-5 hechas y verificadas; siguen las 6-10 (Diagramas de flujo con Mermaid, Descomposición, Arreglos, Algoritmos clásicos, Coste) y el capstone "del pseudocódigo al Python", que es el único de Track 0 con `hidden_tests` normales porque es el puente a Track 1. Faltan también los cuatro tipos de ejercicio que esas lecciones necesitan: `order_steps`, `find_bug`, `flowchart_match` y `flowchart_fill` (los dos últimos con Mermaid, que aún no es dependencia). Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
