...
- ✅ **Track 0 (Fundamentos) cerrado (2026-09-14)** — el tramo de entrada para quien no ha programado nunca, con **ejercicios que no se ejecutan** (pseudocódigo y trazas no corren en Pyodide). Infra hecha: migración **0016** (`exercises.exercise_type` + `spec` público + `answer_key` privada), corrección **en el backend** `POST /exercises/{id}/check` con `track0_service` (determinista, sin Pyodide ni LLM; el doc pedía validar en el cliente y es incompatible con no filtrar la solución), y `ExerciseRunner` en el front que despacha por tipo. Tipos vivos: `trace_table` (el central), `predict_output` y `mcq`. Aprobar crea una `CodeSubmission` normal, así que XP/progreso/competencias **no se ramifican**. Categoría `algoritmos` (no `fundamentos`: ya la usa Track 1) y órdenes **negativos** (-10..-8) para ir antes de Track 1. **10 lecciones** (algoritmo, variables, **traza**, condicionales, bucles, diagramas de flujo, descomposición, arreglos, algoritmos clásicos y coste) con 60 ejercicios, **más el capstone `track-0-del-pseudocodigo-al-python`**: 8 requisitos en Python con `hidden_tests` en Pyodide y certificado de Track 0. Como Track 0 nunca enseña sintaxis, el enunciado del capstone lleva dentro la **tabla de traducción pseudocódigo → Python**. Cinco tipos de ejercicio: `trace_table` (el central), `predict_output`, `mcq`, `flowchart_match` y `flowchart_fill`. Los diagramas se dibujan en **SVG a partir de datos**, no con Mermaid: ver el porqué en [docs/TRACK_0.md](docs/TRACK_0.md). El **tutor** distingue el track: con `track-0` pregunta por la traza, no envuelve la respuesta en un bloque `python` y su fallback no puntúa código a quien no lo escribe. Los arreglos se indexan **desde 0**, no desde 1 como PSeInt: el capstone pide implementar en Python lo ya trazado y cambiar de base en ese salto convierte el puente en un tropiezo. Convención de pseudocódigo: estilo PSeInt en español, **confirmada por el usuario** (cambiarla obliga a reescribir los enunciados). Detalle en [docs/TRACK_0.md](docs/TRACK_0.md).
...
**Contenido en números**: 58 lecciones (Track 0: 10 · Track 1: 10 · Track 2: 11 · Track 3: 11 · Track 4: 5 · Track 5: 6 · Track 6: 5) · 258 ejercicios (198 de código + 60 de Track 0, que no se ejecutan) (todos con hidden_tests; Track 1 reescrito entero: 60) · 100 puzzles ELO curados · 85 retos (25 problemas × 3 niveles, 5 de ellos de datos/ML, + 10 curados; todos con hidden_tests y solución de referencia) · 7 capstones · 3 datasets. Migraciones 0001-0016 (Tracks 3-6 y el proxy LLM **no** añaden migraciones; la 0015 es `coding_challenges.hidden_tests` y la 0016 los tipos de ejercicio de Track 0). 233 tests backend. Esquema de datos: `docs/DATABASE.md`. **Nota**: el contenido real vive en `lesson_seed.py`; `lesson_content.py` es código **muerto/duplicado** (no se importa) — no editarlo pensando que seedea.
...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Track 5 quedó cerrado con el capstone "Nebula RAG". **Track 6 cerrado** (MLOps 1-5 + capstone "pipeline de producción"). **Track 0 cerrado**: 10 lecciones + 60 ejercicios + capstone, todo verificado, y con él **los siete tracks del norte están completos**: de no saber programar a MLOps. Sin trabajo de contenido pendiente asignado. El **tutor de Track 0** quedó hecho el mismo día. Lo que queda apuntado del track son extras que nadie ha pedido: el **diagnóstico de entrada** (5-6 ejercicios que recomienden por dónde empezar, sin bloquear), el Q&A del WebSocket —que es un chat general sin contexto de lección y no sabe del track— y los dos tipos de ejercicio que no han hecho falta (`order_steps`, `find_bug`). Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
...
El **editor tiene tres modos**: libre (`/editor`), lección (`/editor?lesson=<id>&exercise=<id>`) y reto (`/editor?challenge=<id>&dificultad=<filtro>`). En los dos últimos la URL identifica lo que se está resolviendo (compartible/recargable) y hay **Anterior/Siguiente sin salir del editor**: en lección, entre los ejercicios de esa lección (cabecera "Ejercicio N de M"); en reto, entre los retos de la lista con la que se entró (cabecera "Reto N de M"). El `dificultad` de la URL no es decorativo: es el filtro activo en la página de Retos, y es lo que permite reconstruir **esa misma lista** para saber cuál es el siguiente. Sin él, "siguiente reto" no significaría nada. Si la lista no carga, los botones se deshabilitan y el reto se resuelve igual.
