...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Track 5 quedó cerrado con el capstone "Nebula RAG". **Track 6 cerrado** (MLOps 1-5 + capstone "pipeline de producción"). **Track 0 en piloto**: infra + lecciones 1-5, 7 y 8 hechas y verificadas; siguen la 9 (Algoritmos clásicos) y la 10 (Coste), que salen con los tipos actuales, la 6 (Diagramas de flujo), que **necesita Mermaid** —aún no es dependencia— y los tipos `flowchart_match`/`flowchart_fill`, y el capstone "del pseudocódigo al Python", único de Track 0 con `hidden_tests` normales porque es el puente a Track 1. Quedan sin usar `order_steps` y `find_bug`. La lección N va en el orden **N - 11** (de -10 a 0), con su hueco reservado aunque se escriban desordenadas. Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
