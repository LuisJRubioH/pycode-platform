...
**Contenido en números**: 48 lecciones (Track 1: 10 · Track 2: 11 · Track 3: 11 · Track 4: 5 · Track 5: 6 · Track 6: 5) · 198 ejercicios (todos con hidden_tests; Track 1 reescrito entero: 60) · 100 puzzles ELO curados · 85 retos (25 problemas × 3 niveles, 5 de ellos de datos/ML, + 10 curados; todos con hidden_tests y solución de referencia) · 6 capstones · 3 datasets. Migraciones 0001-0015 (Tracks 3-5 y el proxy LLM **no** añaden migraciones; la 0015 es `coding_challenges.hidden_tests`). 213 tests backend. Esquema de datos: `docs/DATABASE.md`. **Nota**: el contenido real vive en `lesson_seed.py`; `lesson_content.py` es código **muerto/duplicado** (no se importa) — no editarlo pensando que seedea.
...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Track 5 quedó cerrado con el capstone "Nebula RAG". **Track 6 cerrado** (MLOps 1-5 + capstone "pipeline de producción"), y con él los seis tracks del norte estratégico: el pipeline Fundamentos → Python → DS → ML → DL → AI Engineering → MLOps está completo. Sin trabajo de contenido pendiente asignado. Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
