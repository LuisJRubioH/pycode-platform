...
- 🚧 **Track 6 (MLOps) en curso** — mismo enfoque "desde cero en el navegador" que Tracks 4-5 (Python/numpy en Pyodide, sin Docker/MLflow reales; las herramientas reales, si entran, como paso opcional no evaluado). Categoría `mlops`. **MLOps 1 · Reproducibilidad** (2026-09-13, estándar de la plantilla: 6 ejercicios, 90% de líneas anotadas): semillas con generador propio, división train/test reproducible, huellas SHA-256 de datos y configuración en formato canónico (`json.dumps(sort_keys=True)`), manifiesto con entorno y métricas, comparación de manifiestos y `ejecutar_experimento` con id = huella de la receta (sin métricas). **MLOps 2 · Seguimiento de experimentos y registro de modelos** (MLflow desde cero): runs con parámetros/métricas/historial y estado, JSON Lines, mejor run (solo terminados con la métrica; empate = el primero), modelo con `pickle` + checksum SHA-256 comprobado **antes** de `pickle.loads`, registro de versiones con etapas (una sola en producción, rollback promoviendo una anterior) y `experimentar` que tolera entrenamientos fallidos y deja el mejor en staging. Plan: MLOps 3 servir un modelo (contrato de entrada y validación) → MLOps 4 monitoreo y drift → MLOps 5 CI/CD de modelos (quality gates, champion/challenger, rollback) → capstone "pipeline de producción".

**Contenido en números**: 45 lecciones (Track 1: 10 · Track 2: 11 · Track 3: 11 · Track 4: 5 · Track 5: 6 · Track 6: 2) · 180 ejercicios (todos con hidden_tests; Track 1 reescrito entero: 60) · 100 puzzles ELO curados · 85 retos (25 problemas × 3 niveles, 5 de ellos de datos/ML, + 10 curados; todos con hidden_tests y solución de referencia) · 5 capstones · 3 datasets. Migraciones 0001-0015 (Tracks 3-5 y el proxy LLM **no** añaden migraciones; la 0015 es `coding_challenges.hidden_tests`). 213 tests backend. Esquema de datos: `docs/DATABASE.md`. **Nota**: el contenido real vive en `lesson_seed.py`; `lesson_content.py` es código **muerto/duplicado** (no se importa) — no editarlo pensando que seedea.
...
**Próximo trabajo**: el plan de correcciones está cerrado (Bloques 0-7, P1 del barrido e issue #32). Track 5 quedó cerrado con el capstone "Nebula RAG". Track 6 (MLOps) en curso: MLOps 1-2 hechas, sigue MLOps 3 (servir un modelo: contrato de entrada y validación). Pendiente de decisión: si el estándar de la plantilla se aplica a las lecciones existentes de Tracks 2-5 (AI 1-3 incluidas). Decisión pendiente aparte: Track 4b con PyTorch real (GPU remota vs Colab). Ver `docs/ARCHITECTURE.md` (diseño), `docs/DATABASE.md` (esquema) y `project_track5_piloto` / `project_track4_piloto` en memoria para el detalle vivo.
